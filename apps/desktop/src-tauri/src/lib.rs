...
fn toggle_popout(app: &AppHandle, rect: &tauri::Rect) {
    ...
    if let Some(monitor) = monitor {
        ...
        // process's life (the CSS viewport then collapses, which no amount of
        // hiding and showing repairs). Deriving from the configured logical
        // size and the target monitor's LIVE scale factor is the one
        // derivation that cannot inherit that drift (tauri #7139/#7890).
        //
        ...
        //
        // The WIDTH is the configured one; the HEIGHT follows the optional
        // rows (map #168 M3, `popout_content_height`): the model-scoped limit
        // row the last readout poll reported, and the accent update row while
        // an update is pending. This is the one place that already re-asserts
        // geometry on every open.
            ...
            .map(|(w, _)| (w, popout::popout_content_height(model_row, update_pending)))
        {
            ...
            let place = || {
                // The unit is the platform's, not ours (popout::InnerSize):
                // Windows is held to physical because its per-window scale
                // factor is the value that goes stale; macOS is held to points
                // because its MONITOR scale factor is the value that goes wrong
                // — tao answers 1.0 when its NSScreen lookup misses, and the
                // physical path then hands AppKit a size it halves.
                ...
                // the last resort.
                // On macOS that scale is now load-bearing for the ANCHOR only:
                // if it lies, the popout lands off-centre by up to half its
                // width, which is a cosmetic miss rather than the unreadable
                // window it used to cause.
